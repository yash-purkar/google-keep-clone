  ...
  return (
    <>
      <div className='flex flex-col items-center justify-center'>
        <input type="text" name="" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Title.." className=" w-64 border h-10  border-yellow-500 rounded-xl px-3 tracking-wider placeholder-gray-500 text-gray-900 focus:outline-yellow-600 sm:w-80 md:w-96" maxLength={15} />

        <textarea name="" id="" cols="30" rows="5" value={content} onChange={(e) => setContent(e.target.value)} placeholder="Write a note..." className=" w-64 p-4 border-2 border-yellow-500 border-t-0 rounded-lg placeholder-gray-400 tracking-wider mb-6 text-gray-900 focus:outline-yellow-600 resize-none sm:w-80 md:w-96" maxLength={28} ></textarea>
          ...
          return (
            <>
              <div key={elem.id} className="flex justify-center flex-col items-center  m-auto border border-yellow-500 w-56 mt-10 bg-white px-3 rounded-md h-28 sm:w-72 ">
                ...
                <p className='bg-white w-full text-center px-5 pt-2  py-2 tracking-wider font-serief'>{elem.content}</p>

                <button onClick={() => deleteNote(elem.id)} className=" 
         px-3 relative w-11 sm:w-12"><img src={remove} alt="remove" className='bg-white border rounded-md border-red-500' /></button>
